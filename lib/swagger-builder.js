...
Builder.prototype.addRoute = function(details, override) {
  ...
  if (override) override = JSON.parse(JSON.stringify(override));
  var route = path[details.method] = override || {};
  route.description = route.description || getRouteDescription(details);
  if (!route.parameters) {
    route.parameters = [];
    var schema = this.swagger.definitions[details.collection] || {properties: {}};
    ...
    pathParams.forEach(function(name) {
      ...
      parameter.description = getParameterDescription(details, parameter);
...
var getParameterDescription = function(opts, parameter) {
  if (parameter.in === 'path' || parameter.in === 'query') {
    if (opts.many) {
      return 'Filter by ' + parameter.name;
    } else {
      return 'Select by ' + parameter.name;
    }
  } else if (parameter.in === 'body') {
    if (opts.method === 'post') {
      return getCollectionDescription(opts) + ' to create.';
    } else if (opts.method === 'put') {
      return getCollectionDescription(opts) + ' to update or create.'
    } else if (opts.method === 'patch') {
      return opts.collection + ' fields to update.'
    }
  }
}
